...
for t in range(DT + 1):
  ...
  # Extract the needed data sets at the specified integer indices
    # Ux = Gf[time + '/Ux'][z_middle - 1, :, :]
    # Uy = Gf[time + '/Uy'][z_middle - 1, :, :]
    # Uz = Gf[time + '/Uz'][z_middle - 1, :, :]
    Ux_ave = Gf[time + '/Ux_TimeAv'][z_middle - 1, :, :]
    Uy_ave = Gf[time + '/Uy_TimeAv'][z_middle - 1, :, :]
    Uz_ave = Gf[time + '/Uz_TimeAv'][z_middle - 1, :, :]
    UxUx_TimeAv = Gf[time + '/UxUx_TimeAv'][z_middle - 1, :, :]
    UyUy_TimeAv = Gf[time + '/UyUy_TimeAv'][z_middle - 1, :, :]
    UzUz_TimeAv = Gf[time + '/UzUz_TimeAv'][z_middle - 1, :, :]

    # Save the data to a .txt file with a time-specific name
    output_file = 'Rxx3DhillN10__H15_Z15_dt0.001_F0.2_' + time + '.txt'
    with open(output_file, 'w') as f:
        f.write('Ux Uy Uz Ux_ave Uy_ave Uz_ave UxUx_TimeAv UyUy_TimeAv UzUz_TimeAv\n')
        np.savetxt(f, np.column_stack(( Ux_ave, Uy_ave, Uz_ave, UxUx_TimeAv, UyUy_TimeAv, UzUz_TimeAv)))
